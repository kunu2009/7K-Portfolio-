...
const Game = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full mobile-padding">
        <div className="w-full max-w-lg mb-4 flex justify-between items-center gap-2">
             <h1 className="font-headline text-mobile-2xl font-bold">Bounce Up!</h1>
             <div className="text-right shrink-0">
                <p className="font-mono text-mobile-sm">HEIGHT: {score}</p>
                 <p className="text-xs text-muted-foreground">Platforms: {platforms.length}/{MAX_PLATFORMS}</p>
             </div>
        </div>
        <div 
            ref={gameAreaRef} 
            className="relative w-full max-w-lg h-[500px] sm:h-[600px] md:h-[700px] bg-secondary/30 rounded-lg overflow-hidden border cursor-crosshair touch-manipulation"
            onClick={placePlatform}
        >
            {gameOver ? (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-20 animate-fade-in-up mobile-padding">
                    <h2 className="text-mobile-3xl font-bold text-red-500 font-mono">Game Over</h2>
                    <p className="text-mobile-lg text-white">Final Height: {score}</p>
                    <p className="text-muted-foreground mb-4 text-mobile-sm">(The ball fell)</p>
                    <Button onClick={resetGame} className="btn-mobile">
